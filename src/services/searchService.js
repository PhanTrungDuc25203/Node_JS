...
let searchWithFallback = async (
    ...
    const { useFullName = false, fullNameCols = ["firstName", "lastName"] } = options;
    ...
    if (useFullName && fullNameCols.length >= 2) {
        caseParts.push(`WHEN CONCAT(${fullNameCols[1]}, ' ', ${fullNameCols[0]}) LIKE '%${searchterm}%' THEN 1`);
    }
    ...
    if (useFullName && fullNameCols.length >= 2) {
        whereOr_exact.push(Sequelize.where(Sequelize.fn("concat", Sequelize.col(fullNameCols[1]), " ", Sequelize.col(fullNameCols[0])), { [Op.like]: `%${searchterm}%` }));
    }
    ...
    const whereOr_soundex = [...fields.map((field) => Sequelize.where(Sequelize.fn("SOUNDEX", Sequelize.col(field)), Sequelize.fn("SOUNDEX", searchterm)))];

    if (useFullName && fullNameCols.length >= 2) {
        whereOr_soundex.push(Sequelize.where(Sequelize.fn("SOUNDEX", Sequelize.fn("concat", Sequelize.col(fullNameCols[0]), " ", Sequelize.col(fullNameCols[1]))), Sequelize.fn("SOUNDEX", searchterm)));
    }
    ...
    if (useFullName && fullNameCols.length >= 2) {
        whereOr_prefix.push(Sequelize.where(Sequelize.fn("concat", Sequelize.col(fullNameCols[0]), " ", Sequelize.col(fullNameCols[1])), { [Op.like]: `${searchterm}%` }));
    }
...
let searchSpecialty = (searchterm) => searchWithFallback(db.Specialty, searchterm, ["name", "markdownDescription"]);

let searchExamPackage = (searchterm) => searchWithFallback(db.ExamPackage_specialty_medicalFacility, searchterm, ["name", "markdownDescription", "markdownCategory"]);

let searchComplexFacility = (searchterm) => searchWithFallback(db.ComplexMedicalFacility, searchterm, ["name", "address", "markdownDescription", "markdownEquipment"]);
        ...
        return {
            ...
            data: userSearch.data.map((u) => ({
                ...
                User: {
                    ...
                    Doctor_specialty_medicalFacility: u.Doctor_specialty_medicalFacility || [],
                },
            })),
        };
    }
...
let allMedicalServiceFilterSearchService = async (searchterm, filter) => {
    try {
        ...
        if (filter) {
            ...
            let [specialty, examPackage, complexFacility, doctor] = await Promise.all([searchSpecialty(searchterm), searchExamPackage(searchterm), searchComplexFacility(searchterm), searchDoctor(searchterm)]);
