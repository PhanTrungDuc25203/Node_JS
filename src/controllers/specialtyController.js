...
let getRemoteSpecialtyForHomePage = async (req, res) => {
    try {
        let response = await specialtyService.getRemoteSpecialtyForHomePageService();
        return res.status(200).json(response);
    } catch (e) {
        console.log(e);
        return res.status(200).json({
...
let getSpecialtyById = async (req, res) => {
    try {
        let infor = await specialtyService.getSpecialtyByIdService(req.query.id, req.query.location);
        return res.status(200).json(infor);
    } catch (e) {
        console.log(e);
